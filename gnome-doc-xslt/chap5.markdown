...
예를 들어 DocBook의  _email_ 요소는 사용자가 주어진 주소로 이메일을 보낼 수 있게 하는 파이퍼 연결로 변환됩니다. 이 형식 구성자에 대한 세르비아어 번역은 아래와 같습니다.
...
각각의 풍선 도움말 형식 구성자는 각자의 형식 인저를 가질 것입니다. 일반적으로 단일 형식 인자가 필요할 것이며 번역자분들은 문자열에 대한 주석을 유효한 인자에 대해 분명하고 확실하게 적어야 할 것입니다.
